    ...
    try {
      ...
      if (token != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Livre ajouté à votre bibliothèque !"),
            ),
          );
        }
      }
      ...
      if (mounted) {
        ...
        if (e.response?.statusCode == 409) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Ce livre est déjà dans votre bibliothèque"),
            ),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Erreur lors de la sauvegarde : ${e.message}"),
            ),
          );
        }
      }
